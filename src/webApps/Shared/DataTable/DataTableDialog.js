...
class DataTableDialog extends React.Component {
  constructor(props) {
    super(props);
    const {
      open,
  ...
  componentDidMount() {
    ...
    this.props
      ...
      .then((response) => {
        const dataCount = response.data.dataCount 
        ? response.data.dataCount
        : response.data.length 
            ? response.data[0].dataCount
            : 0;
        this.setState({
  ...
  render() {
    const {open, multipleSelect, enableCreate, createData } =
      this.state;
...
export default withTranslation()(withStyles(CustomStyles)(DataTableDialog));
