...
class MusteringUsers extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: true,
  ...
  loadUsers = (isSearch) => {
    ...
    ApiHandler.AMSuite.User.GetUsers({
      ...
      .then(({ data }) => {
        this.setState({
          users: data.data.filter((name)=>name.musteringApp==true),
          usersCount: data.data.filter((name)=>name.musteringApp==true).length,
          isLoading: false,
        });
...
MusteringUsers.propTypes = {
  classes: PropTypes.object.isRequired,
  theme: PropTypes.object.isRequired,
};

const MUsersConnected = connect(mapStateToProps, null)(MusteringUsers);

export default withTranslation()(
  withStyles(styles, { withTheme: true })(MUsersConnected)
);
