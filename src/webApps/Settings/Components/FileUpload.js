...
export class FileUpload extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    const { t } = this.props;
    if (this.props.mssgError && prevProps.mssgError !== this.props.mssgError) {
      SnackbarHandler.showMessage(t(this.props.mssgError), "error");
    }
    if (
      this.props.successUpdatedTable &&
      prevProps.successUpdatedTable !== this.props.successUpdatedTable
    ) {
      SnackbarHandler.showMessage(t("CardHoldersSuccessfullyUpdated"));
  ...
  onDrop = (files, rejectedFiles) => {
    ...
    if (rejectedFiles.length !== 0) {
      SnackbarHandler.showMessage(t("InvalidFileFormat"), "error");
    } else {
      this.setState({
          ...
          >
            ...
            <div style={{ padding: 16 }}>
              {this.props.isLoading === true ? (
                ...
                >
                  <Typography variant="h4">{t("LoadingData")}</Typography>
                  <PulseLoader
                    css={override}
                    size={25}
                ...
                <>
                    ...
                    <div className={classes.divAlignment}>
                      ...
                      >
                        <g>
                          <g
                            fill={
                              isValueEmptyOrNull(this.state.file)
                                ? "#666462"
                                : "#fff"
                            }
                          >
                      ...
                      {
                        <Transition
                          visible={!isValueEmptyOrNull(this.state.file)}
                          animation="scale"
                          duration={500}
                        >
                          <Icon
                  ...
                  <Button
                    disabled={isValueEmptyOrNull(this.state.file)}
...
const mapStateToProps = ({ DataBase }) => {
  return {
    ...
    mssgError: DataBase.mssgError,
    successUpdatedTable: DataBase.successUpdatedTable,
  };
};
...
export default withTranslation()(
  withStyles(CustomStyles, { withTheme: true })(connectedFileUpload)
);
